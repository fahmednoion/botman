...
// Parent and Sub-parent management
const PARENT_USERS = new Set(
  (process.env.PARENT_USERNAMES || "faysal,noion,fa.")
    .split(",")
...
// Helper function to check parent/sub-parent
...
async function getAIReply(question) {
  if (!MISTRAL_API_KEY) {
    return "AI is not configured. Please set MISTRAL_API_KEY in .env";
  }
...
class BotAccount {
  constructor({ username, password, token, isMain = false }) {
    ...
    this.floodMode = "custom"; // "custom" or "auto"
  ...
  async login() {
    if (this.token && !this.isTokenExpired()) {
      try {
        ...
        this.log(`✓ Token valid — user: ${payload.username}, ID: ${this.userId}, expires: ${exp}`);
        return true;
      } catch (e) {
    ...
    if (!this.password) {
      this.log("❌ No password set and token is missing/expired");
      return false;
    }
    ...
    try {
      ...
      if (!this.token) {
        this.log(`❌ Login failed (${resp.status})`);
        return false;
      }
  ...
  // ── Flood System ──────────────────────────────────────────
  // Custom text flood: |flood 228 $user Hello World!
  startCustomFlood(roomId, customText) {
    this.stopFlood(); // Stop any existing flood
    ...
    const sendFloodMessage = () => {
      ...
      // Send next message in 100ms (very fast)
  ...
  // Auto flood from flood.txt: |autoflood 228 $user
  startAutoFlood(roomId) {
    this.stopFlood(); // Stop any existing flood

    // Load messages from flood.txt
    ...
    this.log(`🌊 AUTO FLOOD STARTED in room ${roomId} - ${this.floodMessages.length} messages from flood.txt`);
    ...
    const sendFloodMessage = () => {
      ...
      if (this.floodIndex >= this.floodMessages.length) {
        this.floodIndex = 0; // Loop back to start
      }
  ...
  stopFlood() {
    ...
    if (this.floodActive) {
      this.log(`🛑 FLOOD STOPPED in room ${this.floodRoomId} (${this.floodMessageCount} messages sent)`);
    }
  ...
  statusText() {
    ...
    if (this.autoTextOn) {
      ...
      const atProgress = [...this.autoTextRooms].map((rid) => {
        const current = this.autoTextIndex.get(rid) || 0;
        const total = this.autoTextMessages.length;
        return `R${rid}:${current}/${total}`;
      }).join(", ");
    ...
    let status = `👤 @${this.username}\n`;
    status += `  Connection  : ${this.isConnected ? "🟢 Online" : "🔴 Offline"}\n`;
    status += `  Active rooms: [${rooms}]\n`;
    status += `  Voucher pick: ${this.voucherOn ? "🟢 ON" : "🔴 OFF"}\n`;
    status += `  Auto-welcome: ${this.awOn ? `🟢 ON (rooms: ${awRoomsStr})` : "🔴 OFF"}\n`;
    status += `  Auto-reply  : ${this.autoReplyOn ? "🟢 ON" : "🔴 OFF"}\n`;
    status += `  Auto-text   : ${atStatus}\n`;
    status += `  Flood       : ${floodStatus}\n`;
    status += `  Balance     : ${bal}\n`;
